...
class Dashboard extends React.Component {
  constructor(props) {
    ...
    this.buildIntradayPorfolioValues = this.buildIntradayPorfolioValues.bind(
      this
    );
  ...
  componentDidMount() {
    this.props.allTransactions().then(() => {
      this.buildPortfolio();
    });
  }
    ...
    transactions.forEach(transaction => {
      if (transaction.order_type) {
        if (currentPortfolio[tickers[transaction.company_id]] === undefined) {
          currentPortfolio[tickers[transaction.company_id]] =
    ...
    for (let i = 0; i < datesArray.length; i++) {
      ...
      for (let j = 0; j < transactions.length; j++) {
        ...
        if (transactionDate <= dateConvertedToTime) {
          if (transaction.order_type) {
            if (
              historicPortfolio[dateString][transaction.company_id] ===
              undefined
            ) {
              historicPortfolio[dateString][tickers[transaction.company_id]] =
                transaction.quantity;
            } else {
              historicPortfolio[dateString][tickers[transaction.company_id]] +=
                transaction.quantity;
  ...
  mergePortfolioWithPrice(historicPortfolio, historicPrices) {
    ...
    let historicPortfolioValues = mergeWith(
      historicPortfolioDup,
      historicPricesDup,
      multiplier
    );
  ...
  showValue(e) {
    if (e.activePayload !== undefined) {
      this.setState({
        value: parseFloat(e.activePayload[0].payload.value.toFixed(2))
      });
    }
  }
  ...
  buildIntradayPorfolioValues() {
    ...
    Promise.all(
      portfolioTickers.map(ticker => StocksAPIUtil.getIntradayPrices(ticker))
    ).then(res => {
            ...
            >
              ...
              <Line
                ...
                stroke={
                  ...
                  this.state.portfolioValues[
                    this.state.portfolioValues.length - 1
                  ].value
                    ? "#34D199"
                    : "#f55733"
                }
          ...
          <div className="portfolio-buttons">
            <button
              className={`dash-button${
                this.state.timeFrame === "1D" ? " active" : ""
              }`}
            ...
            <button
              className={`dash-button${
                this.state.timeFrame === "1W" ? " active" : ""
              }`}
              onClick={this.handleClick}
              value="1W"
            >
            ...
            <button
              className={`dash-button${
                this.state.timeFrame === "1M" ? " active" : ""
              }`}
              onClick={this.handleClick}
              value="1M"
            >
            ...
            <button
              className={`dash-button${
                this.state.timeFrame === "3M" ? " active" : ""
              }`}
              onClick={this.handleClick}
              value="3M"
            >
            ...
            <button
              className={`dash-button${
                this.state.timeFrame === "1Y" ? " active" : ""
              }`}
              onClick={this.handleClick}
              value="1Y"
            >
